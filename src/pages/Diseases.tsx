...
import {
  ...
  Calendar, Stethoscope, Download, CheckCircle2, Zap, CloudSun,
  AlertTriangle, Filter, Sparkles, X, ChevronRight, Share2, Layers
} from 'lucide-react';
...
export default function Diseases() {
  ...
  const handleDownloadPDF = (disease: DiseaseItem) => {
    try {
      ...
      doc.setFontSize(16);
      doc.text('Smart Crop Advisory - Crop Health Prescription', 14, 16);
      ...
      const tableData = [
        ...
        ['Weather Triggers', disease.weather_triggers],
        ['Chemical Treatment', `${disease.treatment}\nDosage: ${disease.chemical_dosage}`],
        ['Organic & Bio Treatment', `${disease.organic_solution}\nDosage: ${disease.organic_dosage}`],
        ['Preventive Measures', disease.prevention],
        ['Safety Waiting Period', `${disease.safety_waiting_period_days} Days before harvest`],
      ];

      autoTable(doc, {
        startY: 60,
        head: [['Section', 'Prescription Details']],
        body: tableData,
      ...
      doc.text('Generated by Smart Crop Advisory System - Vidhyakar Techno', 14, 285);
              ...
              <form onSubmit={handleRunDiagnosis} className="grid sm:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="Leaves">Leaves (இலைகள் - Spots, Yellowing, Curling)</option>
                    <option value="Stem">Stem / Bark (தண்டு - Lesions, Rot, Wilting)</option>
                    <option value="Fruit/Grain">Fruit / Grain (காய் / கதிர் - Rot, Holes, Spots)</option>
                    <option value="Root">Root Basin (வேர் - Root Rot, Nematodes)</option>
                    <option value="Flower">Flower Buds (பூங்கொத்து - Dropping, Powdery)</option>
                    <option value="Whole Plant">Whole Plant (முழு செடி - Stunting, Wilting)</option>
                  </select>
                </div>
